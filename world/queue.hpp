...
public:
  ...
  void push(T value) {
    ...
    while (queue.size() >= max_size) {
      space_available.wait_for(
          lock, std::chrono::milliseconds{QUEUE_MAX_LOCK_WAIT_MS});
    }
  ...
  /**
   * Pop an element from the queue. Block until data is available and the
   * predicate is true. Throw a std::runtime_error when the queue is empty while
   * the predicate became false. The element will be moved to the given space.
   */
  template <typename P>
  void pop(
      T &value, P predicate = []() { return true; }) {
    std::unique_lock<std::mutex> lock{mutex};
    while (queue.empty() && predicate()) {
      data_available.wait_for(
          lock, std::chrono::milliseconds{QUEUE_MAX_LOCK_WAIT_MS});
    }
    if (queue.empty()) {
